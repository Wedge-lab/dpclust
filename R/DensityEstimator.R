...
Gibbs.subclone.density.est.1d <- function(GS.data, pngFile, samplename, density.smooth = 0.1, post.burn.in.start = 3000, post.burn.in.stop = 10000, density.from = 0, x.max = 2, y.max = 5, mutationCopyNumber = NULL, no.chrs.bearing.mut = NULL, x.max.cap = 3) {
  ...
  for (i in 3:dim(wts)[2]) {
    wts[, i] <- apply(1 - V.h.cols[, 1:(i - 1)], MARGIN = 1, FUN = prod) * V.h.cols[, i]
  }

  post.ints <- matrix(NA, ncol = post.burn.in.stop - post.burn.in.start + 1, nrow = 512)

  if (is.na(x.max) || !is.finite(x.max)) {
  ...
  # Ensure pi.h.cols and wts are finite to avoid crashes in density()
  clean_density_call <- function(x, weights, adjust, from, to) {
    finite_idx <- is.finite(x) & is.finite(weights)
    if (sum(finite_idx) < 2) {
      # Fallback if too few finite points
      return(list(x = seq(from, to, length.out = 512), y = rep(0, 512)))
    }
    x <- x[finite_idx]
  ...
  dens_res <- clean_density_call(c(pi.h.cols[post.burn.in.start - 1, ]),
    weights = c(wts[post.burn.in.start, ]),
    adjust = density.smooth, from = density.from, to = x.max
  )
  xx <- dens_res$x

  for (i in post.burn.in.start:post.burn.in.stop) {
    post.ints[, i - post.burn.in.start + 1] <- clean_density_call(c(pi.h.cols[i - 1, ]),
      weights = c(wts[i, ]),
      adjust = density.smooth, from = density.from, to = x.max
    )$y
  }
