...
read_gsdata_object <- function(indir, samplename, no.iters, conc_param, cluster_conc) {
  ...
  GS.data$S.i <- as.matrix(data.table::fread(paste(filename_prefix, "_states.csv", sep = ""), row.names = 1))
  GS.data$V.h <- as.matrix(data.table::fread(paste(filename_prefix, "_stickbreaking_weights.csv", sep = ""), row.names = 1))
  GS.data$pi.h <- as.matrix(data.table::fread(paste(filename_prefix, "_discreteMutationCopyNumbers.csv", sep = ""), row.names = 1))
  GS.data$alpha <- as.data.frame(data.table::fread(paste(filename_prefix, "_alpha.csv", sep = ""), row.names = 1))
