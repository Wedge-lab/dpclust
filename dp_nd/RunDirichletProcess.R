...
RunDirichletProcess <- function(mutCount, WTCount, totalCopyNumber, copyNumberAdjustment, mutation.copy.number, cellularity, output_folder, no.iters, no.iters.burn.in, subsamplesrun, samplename, conc_param, cluster_conc) {
...
#   print(sum(copyNumberAdjustment==0))
  ...
  GS.data<-subclone.dirichlet.gibbs(mutCount=mutCount,WTCount=WTCount,totalCopyNumber=totalCopyNumber, copyNumberAdjustment=copyNumberAdjustment, cellularity=cellularity,iter=no.iters,conc_param=conc_param,cluster_conc=cluster_conc)
#   
#   finalStates=GS.data$S.i[no.iters,]
#   finalMus=GS.data$pi.h[no.iters,,]
...
#   GS.data = list()
#   GS.data$S.i = read.csv(paste(output_folder,"/",samplename,"_2D_iters",no.iters,"_concParam",conc_param,"_clusterWidth",1/cluster_conc,"_states.csv",sep=""),row.names=1)
#   GS.data$V.h = read.csv(paste(output_folder,"/",samplename,"_2D_iters",no.iters,"_concParam",conc_param,"_clusterWidth",1/cluster_conc,"_stickbreaking_weights.csv",sep=""),row.names=1)
#   GS.data$pi.h = read.csv(paste(output_folder,"/",samplename,"_2D_iters",no.iters,"_concParam",conc_param,"_clusterWidth",1/cluster_conc,"_discreteMutationCopyNumbers.csv",sep=""),row.names=1)
  ...
  if (ncol(mutCount) > 1) {
    for(i in 1:(length(subsamplesrun)-1)){
      for(j in (i+1):length(subsamplesrun)){
        ...
        Gibbs.subclone.density.est(mutation.copy.number[,c(i,j)]/copyNumberAdjustment[,c(i,j)],GS.data,imageFile, post.burn.in.start = no.iters.burn.in, post.burn.in.stop = no.iters, samplenames = paste(samplename,subsamplesrun[c(i,j)],sep=""),indices=c(i,j))		
      }
    }
